...
class Attention(nn.Module):
    def __init__(self, num_patch, num_head, embedding_dim, dim_head):
        super().__init__()
...
class MLP(nn.Module):
    def __init__(self, num_patch, embedding_dim):
        super().__init__()
...
class ViT(nn.Module):
    def __init__(self,embedding_dim, patch_dim, patch_size, num_patch, num_head, dim_head,dropout, channel,num_classes ):
        super().__init__()
...
#         self.encoder4=Encoder(num_patch, num_head, embedding_dim, dim_head,r)
        ...
        for i in range(1, self.block+1):
            setattr(self, f'encoder{i}', Encoder(num_patch, num_head, embedding_dim, dim_head))
